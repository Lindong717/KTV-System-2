...
namespace KTV_management_system
{
    ...
    {
        private static string sql = @"select [Grade_number], [Rank_name] from [dbo].[Waiter_type] where 1=1";
        ...
        void select()
        {
            DbHelper.skinDataGridView(skinDataGridView1, "select [Grade_number], [Rank_name] from [dbo].[Waiter_type]", "");
            DbHelper.skinDataGridView(skinDataGridView2, @"select [Waiter_number], [Waiter name], [Jane_spelling], [sex], [Rank_name], [Contact], [identity card], [type_Name],[description] from [dbo].[Waiter] a
            join [dbo].[Waiter_type] b on a.level=b.[Grade_number]
        ...
        private void Waiter_Load(object sender, EventArgs e)
        {
            ...
            DbHelper.skinCollections(skinComboBox1, "select [Grade_number], [Rank_name] from [dbo].[Waiter_type]", "Grade_number", "Rank_name", "请选择");
        ...
        private void skinButton3_Click(object sender, EventArgs e)
        {
            ...
            if (i <= 0)
            {
                DbHelper.executeNonQuery($@"delete from [dbo].[Waiter_type] where [Grade_number]= '{skinDataGridView1.Rows[skinDataGridView1.CurrentCell.RowIndex].Cells["Column1"].Value}'");
            }
            else
            {
        ...
        private void skinComboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    tmp += $" and a.[Grade_number] = '{skinComboBox1.SelectedValue}'";
                    sx();
                    return;
                }
                sx();
